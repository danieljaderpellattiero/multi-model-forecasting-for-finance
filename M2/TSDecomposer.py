...
class TSDecomposer:
    images_path = './images/data-preprocessing'
    ...
    def plot_components(self, imfs, residue, time_axis, ticker, test_run, phase=1) -> None:
        ...
        images_path = f'{self.images_path}/{ticker}'
        ...
        for imf_index, imf in enumerate(imfs):
            axis[imf_index].plot(time_axis, imf, label=f'imf_{imf_index}', color=self.data_plot_colors[0])
            axis[imf_index].legend(loc='best')
            axis[imf_index].grid(True)
        axis[len(imfs)].plot(time_axis, residue, label='residue', color=self.data_plot_colors[1])
